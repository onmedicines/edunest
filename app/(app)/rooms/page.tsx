...
export default async function RoomsPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--zen-bg)" }}>
      {/* Header */}
      <header className="zen-card rounded-none border-x-0 border-t-0 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-xl">📚</span>
          <span className="font-semibold text-lg" style={{ color: "var(--zen-text)" }}>Study Room</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-sm" style={{ color: "var(--zen-muted)" }}>
            {username}
          </span>
          <form action={logout}>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="zen-card p-6 space-y-3">
            ...
            <p className="text-sm" style={{ color: "var(--zen-muted)" }}>Enter a 6-character room code to join someone's session.</p>
            <JoinRoomForm />
          </div>
        </div>
          ...
          <div className="zen-card p-6 space-y-4">
            ...
            <div className="space-y-2">
              {rooms.map((room: Room) => (
                ...
                >
                  <div>
                    <p className="font-medium text-sm" style={{ color: "var(--zen-text)" }}>{room.name}</p>
                    {room.description && (
                      <p className="text-xs mt-0.5" style={{ color: "var(--zen-muted)" }}>{room.description}</p>
                    )}
                  </div>
                  <span className="text-xs font-mono font-semibold px-2 py-1 rounded" style={{ background: "var(--zen-sage-light)", color: "var(--zen-sage-dark)" }}>
                    {room.code}
                  </span>
                </Link>
              ))}
